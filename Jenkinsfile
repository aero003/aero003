...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
		...
		rc = "sfdx ${toolbelt} force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${SFDC_HOST} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${HUB_ORG}"
            println rc
			if (isUnix()) {
